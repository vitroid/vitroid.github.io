...
sub read_bdf_char {
  ...
  while( <$FH> ) {
    chomp; chomp;
    #return 0	if( /ENDFONT/ or /^$/ ); #removed by matto
    return 0	if( /ENDFONT/ ); #added by matto
    if( /^BITMAP/ ) {
    ...
    if( $key eq "STARTCHAR" or $key eq "ENCODING" ) { #modified by matto
      $char_data{$key}	= $val;
    }
    else {
